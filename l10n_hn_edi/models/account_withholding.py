...
class AccountWithholding(models.Model):
    ...
    def _get_default_journal(self):
        ...
        company_id = self._context.get('force_company', self._context.get(
    ...
    journal_id = fields.Many2one('account.journal', domain=lambda self: [(
        'company_id', '=', self.env.company.id)], default=_get_default_journal)
    ...
    move_count = fields.Integer(
        compute="_compute_moves", copy=False, default=0, store=True)
    move_ids = fields.Many2many(
        'account.move', compute="_compute_moves", copy=False, store=True)
    wtax_widget = fields.Binary(compute='_compute_wtax_widget_info')
    #wtax_widget2 = fields.Text(compute='_compute_wtax_widget_info')
   # value = fields.Text()
    # prueba widg
    ...
    l10n_hn_edi_sequence_id = fields.Many2one(
        'ir.sequence', help='Sequence used on the invoice validation.')
        ...
        for rec in self:
           # rec.wtax_widget = json.dumps(False)
            ...
            for wtax in whtax_ids:
                monto = sum([line.wtax_amount for line in rec.line_ids.filtered(
                    lambda r: r.wtax_id == wtax)])
    ...
    def _l10n_hn_edi_validations(self):
        ...
        if not self.journal_id:
            raise UserError(
                _("In order to allow validate the withholding is necessary assign a journal."))
        sequence = self.journal_id.l10n_hn_edi_wh_seq_id
        if not sequence:
            raise UserError(
                _("Please define a sequence for the withholding on the journal."))
        if sequence.l10n_hn_edi_cai_id:
            if sequence.l10n_hn_max_range_number < sequence.number_next_actual:
                raise UserError(
                    _("Cannot be validated with a number bigger than the correlative."))
            if sequence.l10n_hn_min_range_number > sequence.number_next_actual:
                raise UserError(
                    _("Cannot be validated with a number less than the correlative."))
            if self.date < sequence.l10n_hn_edi_cai_id.authorization_date:
                raise UserError(
                    _("Cannot be validated with a date less than the correlative."))
            if self.date > sequence.l10n_hn_edi_cai_id.authorization_end_date:
                raise UserError(
                    _("Cannot be validated with a date bigger than the correlative."))
    ...
    def action_post(self):
        for rec in self:
            ...
            rec.state = 'posted'
            sequence = rec.journal_id.l10n_hn_edi_wh_seq_id.with_context(
                ir_sequence_date=rec.account_date)
    ...
    def action_cancel(self):
        for rec in self:
            ...
            rec.line_ids.mapped(lambda r: r.move_id.sudo(
...
class AccountWithholdingLine(models.Model):
    ...
    withholding_name = fields.Char(
        related='withholding_id.name', store=True, index=True)
    date = fields.Date(related='withholding_id.date',
                       store=True, copy=False, group_operator='min')
    account_date = fields.Date(
        related='withholding_id.account_date', index=True,  store=True,)
    ref = fields.Char(related='withholding_id.ref', store=True,
                      copy=False, index=True, readonly=False)
    parent_state = fields.Selection(
        related='withholding_id.state', store=True, readonly=True)
    journal_id = fields.Many2one(
        related='withholding_id.journal_id', store=True, index=True, copy=False)
    company_id = fields.Many2one(
        related='withholding_id.company_id', store=True, readonly=True)
    ...
    invoice_name = fields.Char(
        related="invoice_id.name", string='#Factura', store=True)
    invoice_ref = fields.Char(related="invoice_id.ref",
                              string='Referencia', store=True)
    base_amount = fields.Monetary(currency_field='currency_id')
    wtax_id = fields.Many2one('account.tax', string='Withholding Name', domain=[
                              ('type_tax_use', '=', 'none')])
    wtax_code = fields.Char(related="wtax_id.description",
                            string='Withholding Code', store=True,)
    wtax_amount = fields.Monetary(
        'Monto Retención', currency_field='currency_id', )
    wtax_rate = fields.Float('Withholding rate',)
    move_id = fields.Many2one(
        comodel_name='account.move', string='Asiento contable',)
    partner_id = fields.Many2one(
        related='withholding_id.partner_id', string='Supplier', store=True,)
    tax_id = fields.Char("NIF", related="partner_id.vat", store=True,)
    ...
    def onchange_wtax_id(self):
        ...
        self.account_id = self.wtax_id.invoice_repartition_line_ids.mapped(
            'account_id')
    ...
    def _onchange_wtax_amount(self):
        for line in self:
            line.update(
                {'wtax_amount': self.base_amount * self.wtax_rate / 100})
    ...
    def _prepare_wtholding_moves(self):
        ...
        if rec.currency_id == company_currency:
            ...
            balance = rec.currency_id._convert(
                counterpart_amount, company_currency, rec.company_id, rec.account_date)
